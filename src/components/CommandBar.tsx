...
const ACTIONS: Action[] = [
  {
    ...
    keywords: ["carpeta", "folder", "crear", "nueva"],
    icon: FolderPlus,
    run: () => void createFolderAndRename(ROOT_ID),
  },
  ...
  })),
  {
    ...
    hint: "desde tu computadora",
    keywords: ["subir", "importar", "upload"],
    icon: Upload,
    run: () => importInto(ROOT_ID),
  },
  ...USER_WIDGET_TYPES.map<Action>((t) => ({
    ...
    keywords: [
      ...
      "añadir",
      "agregar",
      WIDGET_META[t].label.toLowerCase(),
      t,
    ],
    icon: LayoutGrid,
  ...
  })),
  {
    ...
    keywords: ["explorador", "archivos", "carpetas", "escritorio"],
    icon: FolderOpen,
    run: () => void dispatch("ui.openFiles"),
  },
  {
    id: "browser",
    title: "Abrir navegador",
    hint: "Google",
    keywords: ["google", "web", "internet", "navegador"],
    icon: Globe,
    run: () => void dispatch("ui.openBrowser"),
  },
  {
    id: "terminal",
    title: "Abrir terminal",
    hint: "lenguaje natural",
    keywords: ["terminal", "consola", "comandos", "shell"],
    icon: SquareTerminal,
    run: () => void dispatch("ui.openTerminal"),
  },
  {
    id: "snap",
  ...
  },
  {
    ...
    keywords: ["salir", "sesion", "sesión", "cambiar usuario", "logout"],
    icon: LogOut,
    run: () => void dispatch("system.logout"),
  },
  {
    id: "trash",
    title: "Abrir papelera",
    keywords: ["papelera", "basura", "trash", "borrados"],
    icon: Trash2,
    run: () => void dispatch("ui.openTrash"),
  },
  {
    id: "apps",
    title: "Apps conectadas",
    hint: "Notion, Slack, Google, GitHub, Spotify…",
    keywords: [
      "apps",
      "conectar",
      "integraciones",
  ...
  },
  {
    ...
    icon: Settings2,
    run: () => void dispatch("ui.openSettings"),
  },
  {
    id: "theme",
    title: "Cambiar tema",
    hint: "sistema, claro, oscuro",
    keywords: ["tema", "oscuro", "claro", "dark", "light", "modo", "noche"],
    icon: SunMoon,
    run: () => void dispatch("ui.theme"),
  },
  {
    id: "undo",
    title: "Deshacer última acción",
    hint: "Ctrl Z",
    keywords: ["deshacer", "undo", "revertir"],
    icon: Undo2,
    run: () => void undoLast(),
  },
];
  ...
  const stop = async () => {
    ...
    try {
      ...
      const text = await transcribe(
        audio,
        resolveKey(settings, "groq"),
        usesSharedKey(settings, "groq"),
      );
      if (text) onText(text);
      else
        useToasts
          .getState()
          .push({
            message: "No entendí el audio. Intenta otra vez.",
            kind: "info",
          });
    } catch (err) {
      useToasts
...
function matchAction(a: Action, q: string): boolean {
  const n = norm(q);
  return (
    norm(a.title).includes(n) || a.keywords.some((k) => norm(k).includes(n))
  );
}
...
export function CommandBar() {
  ...
  const askMesa = (text: string) => {
    if (aiReady) void useSession.getState().send(text);
      ...
      useToasts
        ...
        .push({
          message:
            "Configura tu proveedor de IA en Ajustes › Inteligencia para pedirle cosas a Sky.",
          kind: "info",
        });
    }
  };
  ...
  const query = q.trim();
  const semanticForQuery =
    semantic.query === query && query.length > 0 ? semantic : null;

  // Local meaning search: instant, no provider. Debounced so typing stays cheap.
  const embeddingsReady = useEmbeddings(
    (s) => s.enabled && s.status === "ready",
  );
  ...
  const items = useMemo<Item[]>(() => {
      ...
      ? [
          {
            ...
            run: () => {
              ...
              useToasts
                .getState()
                .push({ message: `${calc.display} copiado`, kind: "info" });
            },
            keepFocus: true,
            keepQuery: true,
          },
    ...
    const flowItems: Item[] = aiReady
      ? flowMatches.map((f) => ({
          ...
          hint:
            f.instructions.length > 60
              ? `${f.instructions.slice(0, 60)}…`
              : f.instructions,
          icon: (
            <Zap className="h-[18px] w-[18px] text-accent" strokeWidth={1.75} />
          ),
          run: () =>
            askMesa(
              `Ejecuta el flujo "${f.name}". Sus pasos son: ${f.instructions}`,
            ),
          keepFocus: true,
        }))
      : [];
    ...
    if (semanticForQuery?.status === "done") {
      ...
      if (hits.length === 0) {
        semanticItems.push({
          key: "sem:none",
          title: "Nada coincide por significado",
          hint: "Índice",
          icon: <ScanSearch className="h-[18px] w-[18px] text-ink-3" />,
          run: () => undefined,
    ...
    const vectorItems: Item[] = vectorForQuery
      ...
      .map((h, i) => ({
        ...
        hint: h.snippet
          ? h.snippet.length > 70
            ? `${h.snippet.slice(0, 70)}…`
            : h.snippet
          : `${Math.round(h.score * 100)}% de afinidad`,
        icon: (
          <ScanSearch
            className="h-[18px] w-[18px] text-accent"
            strokeWidth={1.75}
    ...
    const actionItems: Item[] = matched.map((a) => ({
      ...
      hint: a.hint,
      icon: (
        <a.icon className="h-[18px] w-[18px] text-ink-2" strokeWidth={1.75} />
      ),
      run: a.run,
    }));
    const web: Item = {
      ...
      hint: "Navegador",
      icon: (
        <Globe className="h-[18px] w-[18px] text-ink-2" strokeWidth={1.75} />
      ),
      run: () =>
        void dispatch("ui.openBrowser", isUrl ? { url: query } : { query }),
    };
    const ask: Item = {
      key: "ask",
    ...
    const strongAction =
      matched.length > 0 && norm(matched[0].title).startsWith(nq);
    const head = [
      ...calcItems,
      ...flowItems,
      ...vectorItems,
      ...semanticItems.filter(
        (s) => !vectorItems.some((v) => v.key.slice(4) === s.key.slice(4)),
      ),
    ];
    const tail = meaning ? [meaning, web] : [web];
    if (isUrl) return [...head, web, ask, ...fileItems, ...actionItems];
    if (strongFile || strongAction)
      return [...head, ...fileItems, ...actionItems, ask, ...tail];
    if (aiReady)
      return calc
        ? [...head, ...fileItems, ...actionItems, ask, ...tail]
        : [...head, ask, ...fileItems, ...actionItems, ...tail];
  ...
  useEffect(() => {
    const el = listRef.current?.querySelectorAll("[data-item]")[idx] as
  ...
  // Spotlight: with the keyboard on the bar and no conversation open, the bar takes the middle of the screen and
  // the desktop steps back. Talking to Sky keeps the bar at the bottom, where the conversation unfolds above it.
  const spotlight = focused && !chatOpen;

  return (
    <div
      className={cn(
        "pointer-events-none absolute inset-0 z-[100000] flex flex-col items-center",
        spotlight ? "justify-start pt-[20vh]" : "justify-end pb-5",
      )}
    >
      <AnimatePresence>
        {spotlight && (
          <motion.div
            ...
            className="spotlight-veil pointer-events-auto absolute inset-0"
      ...
      <motion.div
        ...
        className="relative flex w-[min(800px,94vw)] flex-col items-center gap-2"
      >
        <AnimatePresence initial={false}>
          {(minimized.length > 0 || pending.length > 0) && !chatOpen && (
            <motion.div
              ...
              exit={{ opacity: 0, y: 6 }}
              className="pointer-events-auto flex w-full flex-wrap items-center gap-1.5"
            >
              {pending.map((p) => (
                <span
                  key={p.id}
                  className="glass flex items-center gap-1.5 rounded-full py-1 pl-1.5 pr-1 text-[12px] text-ink shadow-soft"
                >
                  {p.part.type === "image" ? (
                    <img
                      src={`data:${p.part.mediaType};base64,${p.part.data}`}
                      alt=""
        ...
        <div className="relative w-full">
          <AnimatePresence>
            ...
            {resultsOpen && (
              <motion.div
                key="results"
                initial={{ opacity: 0, y: spotlight ? -8 : 8, scale: 0.99 }}
                ...
                className={cn(
                  "glass pointer-events-auto absolute inset-x-0 overflow-hidden rounded-2xl shadow-win",
                  spotlight ? "top-full mt-2" : "bottom-full mb-2",
                )}
              >
                <ul
                  ref={listRef}
                  className="scrollbar-thin max-h-[46vh] overflow-y-auto p-2"
                >
                  {items.map((item, i) => (
                    <li key={item.key}>
                      {item.section && (
                        <div className="px-3 pb-1 pt-2 text-[11px] font-medium uppercase tracking-wide text-ink-3">
                          {item.section}
                        </div>
                      )}
                      <button
                      ...
                      >
                        ...
                        {item.hint && (
                          <span className="max-w-[45%] shrink-0 truncate text-[12px] text-ink-3">
                            {item.hint}
                          </span>
                        )}
                        {i === idx && (
          ...
          >
            <button
              ...
              onClick={() =>
                hasTurns && useSession.getState().setOpen(!chatOpen)
              }
              className={cn(
            ...
            <div className="flex shrink-0 items-center gap-0.5">
              <DockButton
                label="Archivos"
                onClick={() => void dispatch("ui.openFiles")}
              >
                <FolderOpen className="h-5 w-5" strokeWidth={1.6} />
              </DockButton>
              <DockButton
                label="Navegador"
                onClick={() => void dispatch("ui.openBrowser")}
              >
                <Globe className="h-5 w-5" strokeWidth={1.6} />
              </DockButton>
              <DockButton
                label="Nueva nota"
                onClick={() => void createFileAndOpen(ROOT_ID, "note")}
              >
              ...
              {apps.map((a) => {
                ...
                return (
                  <DockButton
                    key={a.id}
                    label={a.name}
                    onClick={() => void dispatch("ui.openApp", { app: a.id })}
                  >
                    <AppLogo
                      item={{
                        id: a.id,
                        ...
                        abbr: entry?.abbr ?? a.name.slice(0, 2),
                        entry,
                      }}
                      size={24}
                    />
              ...
              {apps.length > 0 && (
                <div className="mx-0.5 h-6 w-px shrink-0 bg-line-2" />
              )}
              <DockButton
                label="Papelera"
                onClick={() => void dispatch("ui.openTrash")}
                badge={(trashCount ?? 0) > 0}
              >
                <Trash2 className="h-5 w-5" strokeWidth={1.6} />
              </DockButton>
              <DockButton
                label="Ajustes"
                onClick={() => void dispatch("ui.openSettings")}
                badge={!aiReady}
              >
