...
export function CommandBar() {
  ...
  return (
    <div className={cn('pointer-events-none absolute inset-0 z-[100000] flex flex-col items-center', spotlight ? 'justify-start pt-[20vh]' : 'justify-end pb-5')}>
      <AnimatePresence>
        {spotlight && (
          <motion.div
            key="veil"
      ...
      <motion.div layout transition={{ type: 'spring', stiffness: 380, damping: 34 }} className="relative flex w-[min(800px,94vw)] flex-col items-center gap-2">
        ...
        <AnimatePresence>
          ...
          {resultsOpen && (
            <motion.div
              ...
              exit={{ opacity: 0, y: spotlight ? -6 : 6, transition: { duration: 0.12 } }}
              transition={{ type: 'spring', stiffness: 520, damping: 38 }}
              onMouseDown={(e) => e.preventDefault()}
              className={cn('glass pointer-events-auto absolute inset-x-0 overflow-hidden rounded-2xl shadow-win', spotlight ? 'top-full mt-2' : 'bottom-full mb-2')}
            >
